...
export default class BemLevelsView extends SelectListView {
    ...
    viewForActiveItem(item /*:string*/) {
        return this.viewForItem(`<b>${item}</b>`);
    }
    ...
    populateList() {
        ...
        return this._lastWalk = bemWalk(this._levels)
            .on('data', (file) => {
    ...
    showLastList() {
        ...
        for (const fileInfo of this._lastList) {
            const itemView = activeFilePath && activeFilePath === fileInfo.filePath
                ? $(this.viewForActiveItem(fileInfo.relativeLevelPath))
                : $(this.viewForItem(fileInfo.relativeLevelPath));
